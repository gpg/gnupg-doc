...
One widely-used keyserver network was www.sks-keyservers.net.  SKS
stands for “Synchronizing Key Server”.  You can use this network by
supplying the =--keyserver pool.sks-keyservers.net= option.
Keyservers are of less use these days and most of them are not anymore
in service.  It is better to attach the key to a mail or provide keys
using a Web Key Directory.
...
=gpg --output signed_file.asc --local-user= /[your key ID]/ =--clearsign message_file.txt=

To verify the resulting file, invoke GnuPG with the filename of
the signed file:

=gpg --output signed_file signed_file.asc=

The text which was actually signed can be found in the new file
=signed_file= .  See
https://gnupg.org/blog/20251226-cleartext-signatures.html for
more details on pitfalls with that format.
