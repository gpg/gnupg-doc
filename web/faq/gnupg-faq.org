...
GnuPG is free cryptographic software from GNU which helps people
ensure the confidentiality, integrity and assurance of their
...
- /GNU./  GNU is a group that aims to give people the ability to do
  all their computing with free software.  Some groups have made
  entire operating systems from it, but lots of GNU software will run
  on proprietary systems, too — although we encourage you to shift to
  free ones if you can.
...
(3) For best experience make sure to put ~set crypt_use_gpgme~ in your
    =~/.muttrc= file.  Note that this requires installing Mutt or
