...
2.2 on an Ubuntu 19.10 x64 box.  I’m using Thunderbird with
...
[[mailto:rjh@sixdemonbag.org?subject%3DThe%20GnuPG%20FAQ][Rob Hansen]]. Please 
feel free to contact me should there be an error in this FAQ, whether 
typographical, grammatical, or factual.
...
- /Free./ When we say “free” we mean 
  [[https://gnu.org/philosophy/free-sw.html][liberty, not price]].  
  You’re free to use it, modify it, share it, tinker with it, and learn
  from it. Software is meant to be used by people, and people deserve
  freedom.
...
- /GNU Project./  The [[https://www.gnu.org][GNU Project]] is a group that
  aims to give people the ability to do all their computing with free 
  software.
...
- The [[https://www.gpgtools.org][GPGtools project]]
- [[http://sourceforge.net/projects/gpgosx/][GnuPG for OS X]]
...
The Gentoo documentation includes a
[[https://www.gentoo.org/doc/en/gnupg-user.xml][GnuPG User Guide]].
...
operating system,
[[https://www.gnu.org/distros/free-distros.html][you have many choices]].
...
Go to the [[https://www.gnupg.org/documentation/mailing-lists.en.html][GnuPG mailing list page]]
to subscribe or read the archives.

The [[https://bugs.gnupg.org/gnupg/][GnuPG project’s bug tracker]]
...
- Supports PGP/MIME :: Yes
- Languages supported :: English, Deutsch, Schwyzerdütsch
...
However, PGP Corporation has a well-regarded 
[[http://www.cs.unibo.it/babaoglu/courses/security/resources/documents/intro-to-crypto.pdf][/Introduction to Cryptography/]].
...
Probably the best-known organization is the 
[[https://www.eff.org][Electronic Frontier Foundation]], which has 
been at the vanguard of electronic civil liberties for over twenty years.

The [[https://www.fsf.org][Free Software Foundation]] is also deeply
involved in these matters, although in a different way than the EFF.
...
During roughly the same time period that [[http://www.nist.gov][NIST]]
was running the Advanced Encryption Standard trials, Japan’s 
[[http://www.cryptrec.jp/english/][CRYPTREC]] and the European Union’s
[[http://www.cryptonessie.org/][NESSIE]] were running their own similar
trials.  Camellia is the cipher that won the NESSIE and CRYPTREC trials,
much in the same way that Rijndael won the United States’ AES trials.
...
As of 2019, the era of 2048-bit keys is rapidly coming to a close.  If
...
this just by creating a new certificate with =gpg --gen-key=; it
defaults to creating 3072-bit keys.
...
3072-bit RSA keys.  This is the default behavior for GnuPG.
...
In 2019 the SKS keyserver network was attacked by individuals who
wished to leave it beyond use, and largely succeeded.  Today the
best bet going forward is to use the Hagrid keyserver at 
[[https://keys.openpgp.org/][keys.openpgp.org]].  Instructions for
uploading your certificate may be found there.
...
If you need separate configuration files for GnuPG 1.4 and 2.2, name
them =gpg.conf-1= and =gpg.conf-2=.  GnuPG 1.4 will read the =-1= file,
and 2.2 will read the =-2= file.
...
# Ask me for my confidence level before signing someone's cert.
# This is an advanced feature few people will need to use.
ask-cert-level

# Use full 16-character key IDs, not short 8-character key IDs.
keyid-format long

# Use the Hagrid keyserver for certificate lookups.
...
# Always include signatures from these two certificates.  The
# exclamation point here tells GnuPG "use these two keys exactly."
# Otherwise, GnuPG might substitute a different subkey if a newer
# one exists.
local-user 1DCBDC01B44427C7!
local-user A83CAE94D3DC3873!

# Always add these two certificates to my recipients list.
encrypt-to DC0F82625FA6AADE!
encrypt-to AA24CC81B8AED08B!

# Turn "From" into "> From", in order to play nice with UNIX mailboxes.
escape-from-lines

# Always use SHA512 to sign certificates
...
Many people have had excellent luck with =keys.openpgp.org=.
...
** Why did GnuPG default to 2048 bit RSA?
   :PROPERTIES:
...
According to NIST Special Publication 800-57, RSA-2048 is expected to
be secure until 2030.  For this reason, as we approach 2020 the GnuPG
defaults have changed to RSA-3072.
...
properly.  Some mailing lists are incompatible with it and/or mangles 
it (old versions of Mailman, for instance).
